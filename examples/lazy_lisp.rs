                ...
                match &list.as_slice() {
                    [Expression::Symbol(procedure_name), ref args @ ..] => {
                        match procedure_name.as_str() {
                            ...
                            "lambda" => {
                                println!("constructing a lambda with list:{:?}", list);
                                if let [_, Expression::List(params), Expression::List(body)] =
                                    list.as_slice()
                                {
                                    println!(
                                        "defining lambda with params:{:?} body:{:?}",
                                        params, body
                                    );
                                    Some(Expression::Procedure(params.clone(), body.clone()))
                                } else {
                                    println!("unable to construct a lambda");
                                    None
                                }
                            }
                            "eval" => {
                                let result = args
                                    .iter()
                                    .inspect(|x| println!("evaluating {:?}", x))
                                    .map(|x| x.evaluate(env).unwrap())
                                    .collect::<Vec<Expression>>();
                                Some(Expression::List(Rc::new(result)))
                            }
                            ...
                            "define" => {
                                if let [_, Expression::Symbol(name), ref expression] =
                                    list.as_slice()
                                {
                                    env.global.insert(name.as_ref().clone(), expression.clone());
                                }
                                None
                            }
                            ...
                            name => {
                                println!(
                                    "invoking user defined procedure {:?} with arguments {:?}",
                                    name, args
                                );
                                if let Some(result) = env.global.get(name) {
                                    println!("where {:?} is {:?}", name, result);
                                    Some(result.clone())
                                } else {
                                    None
                                }
...
pub fn main() {
    let mut repl = hyoka::Repl::new("lazy-lisp>", Env::new(), |env: &mut Env, y: String| {
        let mut y = y
            .replace('(', " ( ")
            .replace(')', " ) ")
            ...
            .map(String::from)
            .collect::<Vec<String>>();
        if let Ok(Some(result)) = Expression::parse(&mut y)
            .map(|x| x.evaluate(env))
            .map_err(|x| {
                println!("error:{}", x);
            })
        {
            println!("result:{:?} env:{:?}", result, env);
            None
        } else {
            println!("env:{:?}", env);
            None
